...
const setSchema = (pathname) => {
  ...
  const schemas = [{
    ...
    url: absolute("/"), telephone: SITE.phoneSchema, logo: absolute("/images/logo.png"), foundingDate: SITE.since,
    image: [absolute("/images/hero-bg.jpg"), absolute("/images/food4.jpg"), absolute("/images/bar-special.jpg")],
    description: "Authentic Asian-fusion restaurant, cocktail bar and live-entertainment venue on Peralanda Road in Ragama.",
    ...
    slogan: "Authentic Asian Fusion Cuisine", servesCuisine: ["Authentic Asian Fusion", "Asian", "Sri Lankan"], menu: absolute("/menu"), hasMap: SITE.map, acceptsReservations: true,
...
export default function Seo() {
  ...
  useEffect(() => {
    ...
    [["og:title",seo.title],["og:description",seo.description],["og:image",image],["og:image:alt",`${SITE.name} in Ragama`],["og:url",canonical],["og:type","website"],["og:site_name",SITE.name],["og:locale","en_LK"]].forEach(([property,content])=>meta(`meta[property="${property}"]`,{property,content}));
    [["twitter:card","summary_large_image"],["twitter:title",seo.title],["twitter:description",seo.description],["twitter:image",image],["twitter:image:alt",`${SITE.name} in Ragama`]].forEach(([name,content])=>meta(`meta[name="${name}"]`,{name,content}));
